...
options = {"low_memory_version": False}
f = None
...
time = dolfinx.fem.Constant(mesh, 0.)
p_ex = -1/4 * (ufl.cos(2*ufl.pi*x[0])+ufl.cos(2*ufl.pi*x[1]))*ufl.exp(-4*ufl.pi**2*nu*time)
...
while u_ex.t < 0.5:
    ...
    i += 1
    solver.u
    print(u_ex.t, dolfinx.fem.assemble_scalar(L2_u))
    print(u_ex.t, dolfinx.fem.assemble_scalar(L2_p))

    vtxp.write(time.value - dt/2)
    vtxu.write(time.value)
    if i == 2:
        break
